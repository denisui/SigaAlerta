...
class Divers extends CI_Controller {
    ...
    public function index() {
         ...
         */
        $config["base_url"] = base_url() . 'classified/divers/p';
        ...
        $data['data'] = $this->Model_Classified->getNewsCategory('divers','id', 'desc', $config['per_page'], $offset);

        $this->load->view('public/classified/divers/view', $data);
    }

    public function details($id = NULL) {
        if (empty($id)) {            
            redirect(base_url('classified/divers'));
        } else {
            $data['worldNewsMenu'] = $this->Model_News->getNewsCategory('Mundo', 'id', 'desc', '3', '0');
            $data['techNewsMenu'] = $this->Model_News->getNewsCategory('Tecnologia', 'id', 'desc', '3', '0');
            $data['data'] = $this->Model_Classified->_selectByID($id);
            $data['older_data'] = $this->Model_Classified->_getNewRand('6', '0');
            $this->load->view('public/classified/divers/details', $data);
        }        
    }   
}
