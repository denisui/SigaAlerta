...
class Construction extends CI_Controller {
    ...
    public function index() {        
         ...
         */
        $config["base_url"] = base_url() . 'forhome/construction/p';
        ...
        $data['data'] = $this->Model_Forhome->_getCategory('Construção','id', 'asc', $config['per_page'], $offset);

        $this->load->view('public/forhome/construction/view', $data);
    }

    public function details($id = null) {
        if (empty($id)) {
            redirect(base_url('forhome/construction'));
        } else {
            $data['worldNewsMenu'] = $this->Model_News->getNewsCategory('Mundo', 'id', 'desc', '3', '0');
            $data['techNewsMenu'] = $this->Model_News->getNewsCategory('Tecnologia', 'id', 'desc', '3', '0');
            $data['data'] = $this->Model_Forhome->_selectByID($id);
            $data['older_data'] = $this->Model_Forhome->_getNewRand('6', '0');
            $this->load->view('public/forhome/construction/details', $data);
        }
    }
}
