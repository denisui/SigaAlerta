...
class Electrician extends CI_Controller {
    public function __construct() {
        parent::__construct();
        $this->load->model('public/Model_Forhome');
    ...
    public function index() {
        ...
        $this->row = $this->Model_Forhome->countAll();

        /**
         * Configuração de paginação
         */
        $config["base_url"] = base_url() . 'forhome/electrician/p';
        ...
        $data['data'] = $this->Model_Forhome->_getCategory('Eletrecistas','id', 'asc', $config['per_page'], $offset);

        $this->load->view('public/forhome/garden/view', $data);
    }

    public function details($id = null) {
        if (empty($id)) {
            redirect(base_url('forhome/garden'));
        } else {
            $data['worldNewsMenu'] = $this->Model_News->getNewsCategory('Mundo', 'id', 'desc', '3', '0');
            $data['techNewsMenu'] = $this->Model_News->getNewsCategory('Tecnologia', 'id', 'desc', '3', '0');
            $data['data'] = $this->Model_Forhome->_selectByID($id);
            $data['older_data'] = $this->Model_Forhome->_getNewRand('6', '0');
            $this->load->view('public/forhome/garden/details', $data);
        }
    }
}
