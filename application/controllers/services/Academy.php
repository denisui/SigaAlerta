...
class Academy extends CI_Controller {
    ...
    public function index() {
         ...
         */
        $config["base_url"] = base_url() . 'services/academy/p';
        ...
        $data['data'] = $this->Model_Services->_getCategory('Academia e Afins','id', 'asc', $config['per_page'], $offset);

        $this->load->view('public/services/academy/view', $data);
    }

    public function details($id = null) {
        if (empty($id)) {
            redirect(base_url('services/academy'));
        } else {
            $data['worldNewsMenu'] = $this->Model_News->getNewsCategory('Mundo', 'id', 'desc', '3', '0');
            $data['techNewsMenu'] = $this->Model_News->getNewsCategory('Tecnologia', 'id', 'desc', '3', '0');
            $data['data'] = $this->Model_Services->_selectByID($id);
            $data['older_data'] = $this->Model_Services->_getNewRand('6', '0');
            $this->load->view('public/services/academy/details', $data);
        }
    }
}
