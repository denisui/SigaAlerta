...
class Agency extends CI_Controller {
    ...
    public function index() {
         ...
         */
        $config["base_url"] = base_url() . 'services/agency/p';
        ...
        $data['data'] = $this->Model_Services->_getCategory('Agência de Seguros','id', 'asc', $config['per_page'], $offset);

        $this->load->view('public/services/agency/view', $data);
    }

    public function details($id = null) {
        if (empty($id)) {
            redirect(base_url('services/agency'));
        } else {
            $data['worldNewsMenu'] = $this->Model_News->getNewsCategory('Mundo', 'id', 'desc', '3', '0');
            $data['techNewsMenu'] = $this->Model_News->getNewsCategory('Tecnologia', 'id', 'desc', '3', '0');
            $data['data'] = $this->Model_Services->_selectByID($id);
            $data['older_data'] = $this->Model_Services->_getNewRand('6', '0');
            $this->load->view('public/services/agency/details', $data);
        }
    }
}
